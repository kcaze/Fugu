...
public class Level1 : Level {
	public override void Setup() {
		...
		for (int ii = 0; ii < 5; ii++) {
			AddEnemy(0.0f, lw*(ii+1)/6, 3, "Bouncer");
		}
		AddEnemy(0.0f, lw/2, lh-3, "Shooter");
		AddEnemy(0.0f, lw/2-3, lh-3, "Shooter");
		AddEnemy(0.0f, lw/2+3, lh-3, "Shooter");
		...
		for (int ii = 0; ii < 6; ii++) {
			AddEnemy(0.0f, 6+3*Mathf.Cos(Mathf.PI*2*(ii+0.5f)/6), lh-6+3*Mathf.Sin(Mathf.PI*2*(ii+0.5f)/6), "Stander");
		}
		for (int ii = 0; ii < 6; ii++) {
			AddEnemy(0.0f, lw-6+3*Mathf.Cos(Mathf.PI*2*ii/6), 6+3*Mathf.Sin(Mathf.PI*2*ii/6), "Stander");
		}
		NewWave(100);
		for (int ii = 1; ii < 9; ii++) {
